...
| **开始** | 场景开始时间点 | 格式：\`MM:SS\`（如 00:00） |
| **结束** | 场景结束时间点 | 格式：\`MM:SS\`（如 00:05） |
| **时长** | 场景持续时长 | 格式：\`MM:SS\`（如 00:05） |
...
1. **时间格式统一**：统一使用 \`MM:SS\` 格式（分钟:秒），例如 00:15、01:30
2. **时间连续性**：所有片段的时间必须连续，不能有重叠或间隙
3. **时长准确性**：\`时长 = 结束时间 - 开始时间\`，必须精确计算
...
| 序号 | 景别 | 运镜 | 画面内容描述 | 关键花字 | 口播/台词 | 音效/BGM | 开始 | 结束 | 时长 |
|------|------|------|--------------|----------|-----------|----------|------|------|------|
| 1 | 中景 | 固定 | 主持人坐在书桌前，神情兴奋，双手做出夸张的手势。 | 揭秘时刻！ | 兄弟们，今天我们要拆解的这款神器简直绝了！ | 动感鼓点入场 | 00:00 | 00:05 | 00:05 |
| 2 | 特写 | 推 | 镜头快速推进到手机屏幕细节，展示像素排列。 | 4K画质 | 你们看这个细节，是不是连毛孔都看得清？ | 放大音效(Whoosh) | 00:05 | 00:09 | 00:04 |
\`\`\`
